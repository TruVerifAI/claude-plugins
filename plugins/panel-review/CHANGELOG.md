...
**A review now tells you whether it released the gate.** Every gate-bound `audit_coding` /
`deliberate_coding` response carries a `gate_release` field: `released: true` means retry the
blocked action; `released: false` names the exact reason (a PASS is a review whose final *action*
is proceed/proceed_with_caveats — a major finding raises the action past that) and the free next
step (`record_gate_skip(recommendations_applied, gate_context_id)`). Previously a non-releasing
review was silent and you discovered it only by retrying the commit.

- **Applied-findings releases floor at BOTH gates** (rev-3): after ONE review, applying its
  findings and calling `recommendations_applied` releases the change — floor hunks included —
  lineage-verified against your recent review, minutes-TTL, logged distinctly as "findings
  applied". Deferral (`review_deferred_to_commit`) stays write-gate-only.
- **Merge commits get a first-class release**: `branch_already_reviewed` (merge fires only,
  server-enforced) releases non-floor hunks whose per-commit receipts don't hash-match the merge
  diff's re-hunked boundaries.
- **Cosmetic edits no longer re-block**: a normalized coverage tier forgives pure formatting
  drift (an em-dash swapped for a hyphen) after a real release; any semantic change still fires.
- **Every block ends with machine-readable release options** — the exact releasing calls with the
  `gate_context_id` pre-filled.
- **Fail-open is never silent**: if the gates' backend is unreachable they still allow (never trap
  the agent) but now surface a visible advisory, and `/panel-review:setup` gained a free
  gate-endpoint self-check (`gate_selfcheck.py`) that proves the gates' half end-to-end.
- **Review quality**: severity anchors now define all four tiers (a `major` must name a concrete
  failure scenario or it self-demotes to minor; process gaps like missing tests cap at minor), and
  audits/deliberations prefer the simplest change that solves the problem — architecture
  expansions are preference-level notes, never findings.
