...
- **Write gate** — before a risky **Write / Edit** (schema, migration, dependency, auth, IaC exposure, etc.). A Write/Edit is *finished code*, so its natural review is **`audit_coding`** (a PASS releases it) — or a **`synthesize_coding`** SYNTH_CONFIRM for a fast low-risk check; **`deliberate_coding`** is also accepted (prefer it when the design is still open). Both gates release the same way — a review releases either.
...
**Floor classes can't be waved away with a one-line skip.** For the highest-stakes changes — **auth, secrets, money, migrations, and removed guards** — a judgment `record_gate_skip` (false positive, trivial, etc.) is **denied**, and a recent *unrelated* review doesn't release them either (the `recent_pass` shortcut is floor-scoped). Release a floor change by running `audit_coding` (a PASS clears it), or, for a genuine false positive, `synthesize_coding` — if the panel agrees it's low-risk the server mints a cheap **SYNTH_CONFIRM** (~15–30s) that releases the gate. When you run either, **also pass the `gate_context_id` the gate printed**: coverage then binds to the gate's own recorded hunks, so a genuinely-reviewed change clears reliably even if the diff you pass back drifted by a character (a smart-quote, an em-dash) — no more "I reviewed it but it's still blocked." On a floor **write** (Write/Edit) the gate also prints a `target_hunk_hashes` line — pass it too and coverage binds to exactly those floor hunks, releasing deterministically even when the write gate's diff shape differs from your `gate_diff`. (`deliberate_coding` is for non-floor writes and still-open designs.) If the review tool is genuinely down during a *sustained* outage (or a reviewed floor change still can't bind coverage), the gate asks a **human** to approve (the agent can't skip a floor change past it, and can't approve its own prompt). A reason code whose skips keep preceding real findings can also be **suspended** for a repo (off by default; maintainer-enabled on real data) — a suspended skip is denied and you run the review.
