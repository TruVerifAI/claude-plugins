...
When you install this plugin, your Claude Code agent gets nine skills that auto-activate at decision moments, across two profiles — **coding** and **financial**. Per profile, three fire around a decision (audit / deliberate / synthesize); three more are shared (one AFTER any decision, one when a review gate fires, one for defining your repo's custom floor classes):
...
- **`define-custom-floors`** — Fires when you ask for your own domain-critical code to be protected ("changes to our tax rules should always be reviewed") or want `.truverifai/risk.json` created or edited. Guided workflow: the agent interviews you, drafts the file, validates it with `npx @truverifai/init floors check --preview`, shows you exactly which files each floor covers, and saves only once you approve. The resulting **custom floor classes** block exactly like the built-in floors (see Review gates below). Free of credits.
...
**Floor classes can't be waved away with a one-line skip.** For the highest-stakes changes — **auth, secrets, money, migrations, a removed guard, and other high-severity domains** (memory-safety, infra-as-code exposure, TLS/CI security), **plus any custom floor classes your repo defines** (below) — a judgment `record_gate_skip` (false positive, trivial, etc.) is **denied** while any floor hunk is still **unreviewed**, and a recent *unrelated* review doesn't release them either — the recent-pass shortcut is disabled whenever a floor hunk is uncovered. To release a floor change:

**Custom floor classes (yours).** Your repo can declare its own domain-critical floors — "tax rules", "dosing logic", "pricing engine" — in a committed `.truverifai/risk.json` at the repo toplevel. A change matching one blocks exactly like a built-in floor and releases the same way, and the block message quotes **your own description** of why that code is critical. The file never leaves your machine (the gates compile it locally; the server only ever sees category labels), needs no server setup, and applies to every contributor and every gate platform once committed. Each floor is `{name, description, paths/keywords/patterns, exclude_paths?, test_exempt?}`; the `define-custom-floors` skill runs the guided setup (interview → draft → validate with `npx @truverifai/init floors check --preview` → your approval → save), or scaffold by hand with `npx @truverifai/init floors init`.
