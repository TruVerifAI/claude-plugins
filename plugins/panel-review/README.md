...
- **Write gate** — before a risky **Write / Edit** (schema, migration, dependency, auth, IaC exposure, etc.). A Write/Edit is *finished code*, so its natural review is **`audit_coding`** (a PASS releases it); use **`deliberate_coding`** only when the design is still open. On a **floor-class** write, the cheapest release is a free **`confirm_floor`**, then a **`synthesize_coding`** SYNTH_CONFIRM (on a non-floor change synthesize writes no receipt — see Floor classes below). Both gates release the same way — a review releases either.
...
To release a blocked gate, run the suggested tool **or** call `record_gate_skip` to log a skip-with-reason (free; the reason improves the classifier). The block message prints the exact release key to copy — preferably a server-issued `gate_context_id` (`gc_…`), which the server verifies and consumes single-use, using its own recorded hunks/area; an older gate that prints no id falls back to a `hunk_hashes` list (commit gate) or an `area` (write gate). Either way you copy the key, never reconstruct it.
...
- **`gate_tightness`** (`focused` / `thorough`, default `focused`) — how often **both gates** block (the commit gate *and* the write gate now share this one setting). `focused` blocks only the highest-stakes changes — the floor classes (auth, secrets, billing, migrations, a removed safety check — blocked at **any** confidence) and high-confidence security signals — and downgrades lower-confidence "code-review" changes (API routes, concurrency, network calls, large refactors, error handling) to a non-blocking advisory; `thorough` blocks any risky change that isn't covered. The floor always blocks at both levels. (This replaces the retired `deliberate_mode` write-gate setting; an old `deliberate_mode=block` maps to `thorough`, otherwise `focused`.)
