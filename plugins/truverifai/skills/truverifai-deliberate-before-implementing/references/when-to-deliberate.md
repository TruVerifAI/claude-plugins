...
- **Decisions a quick search would answer.** Idiomatic patterns, standard ways to do X — that's `truverifai-synthesize-quick-check` territory.
...
If you ran `truverifai-synthesize-quick-check` first and got `agreement_score < 0.7`, that's a signal to escalate to deliberate. The synthesize result told you the question is harder than it looked.
